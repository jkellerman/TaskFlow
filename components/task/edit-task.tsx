...
export default function EditTask({ title, description, subtasks, status, columns, labels, date }: EditTaskProps) {
	...
	return (
		<Dialog onOpenChange={(open) => !open && setSubTasks(subtasks)}>
			<DropdownMenu>
				...
				<DropdownMenuContent className=" w-[175px]">
					<DialogTrigger asChild>
						<DropdownMenuItem className="group data-[highlighted]:text-primary">
							<Icons icon="Mark" className="h-5 w-5 pr-2 opacity-0 transition  duration-300 group-hover:opacity-100" />
							<span className="-translate-x-4 transition duration-300 group-hover:translate-x-0">Edit Task</span>
						</DropdownMenuItem>
					</DialogTrigger>
					<DropdownMenuItem className="group text-destructive dark:text-destructive ">
						<Icons icon="Mark" className="h-5 w-5 pr-2 opacity-0 transition  duration-300 group-hover:opacity-100" />
